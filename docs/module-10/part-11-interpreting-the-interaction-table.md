...
2. **delta_aic is positive despite significant LR test.** This happens when `n_cells` is large enough that the AIC cost outweighs the deviance gain. The GLM test says the interaction is real; AIC says the model does not benefit enough from it to justify the complexity.
